...
const obtenerVentas = (req, res) => {
  Venta.obtenerTodas((err, resultados) => {
    if (err) return res.status(500).json({ status: 500, message: 'Error al obtener las ventas' });
...
const obtenerVentaPorId = (req, res) => {
  ...
  Venta.obtenerPorId(id_venta, (err, resultados) => {
    if (err) return res.status(500).json({ status: 500, message: 'Error al obtener la venta' });
    if (resultados.length === 0) return res.status(404).json({ status: 404, message: 'Venta no encontrada' });
...
const crearVenta = (req, res) => {
  ...
  Venta.crear(venta, (err, resultado) => {
    if (err) return res.status(500).json({ status: 500, message: 'Error al crear la venta' });
...
const actualizarVenta = (req, res) => {
  ...
  if (!venta.id_venta || !venta.id_vehiculo || !venta.id_cliente || !venta.id_vendedor || !venta.precio_final) {
    return res.status(400).json({ status: 400, message: 'Faltan campos requeridos para actualizar' });
  }

  Venta.actualizar(venta, (err, resultado) => {
    if (err) return res.status(500).json({ status: 500, message: 'Error al actualizar la venta' });
    if (resultado.affectedRows === 0) return res.status(404).json({ status: 404, message: 'Venta no encontrada' });
...
const eliminarVenta = (req, res) => {
  ...
  Venta.eliminar(id_venta, (err, resultado) => {
    if (err) return res.status(500).json({ status: 500, message: 'Error al eliminar la venta' });
    if (resultado.affectedRows === 0) return res.status(404).json({ status: 404, message: 'Venta no encontrada' });
